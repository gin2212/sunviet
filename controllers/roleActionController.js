...
async function createRoleAction(req, res) {
  if (req.actions.includes("role-action")) {
    try {
      ...
      if (role) {
        let exists = await RoleActions.find({
          role: req.body.role,
        ...
        if (strActions) {
          ...
          if (actions.length > 0) {
            ...
            actions.forEach((action, index) => {
              let roleAction = new RoleActions({
                role: req.body.role,
                action: action.trim(),
              });
              roleAction.createdTime = Date.now();
...
function getPagingRoleActionsByUserId(req, res) {
  ...
  if (req.query.userId) {
    Users.findById(req.query.userId, (error, user) => {
      if (error) {
        ...
        if (user) {
          ...
          res
            .status(404)
            .json(new ResponseModel(404, "User was not found!", null));
        }
      }
    });
  } else {
    res
      .status(404)
      .json(new ResponseModel(404, "Not found userId in query params!", null));
  }
}
...
async function getPagingRoleActions(req, res) {
  ...
  try {
    let roleActions = await RoleActions.find(searchObj)
      .skip(pageSize * pageIndex - pageSize)
      .limit(parseInt(pageSize))
      .populate("role")
      .populate("action")
...
async function getAllRoleAction(req, res) {
  try {
    let searchObj = {};
    if (req.query.roleId) {
      searchObj = {
    ...
    res.status(200).json(new ResponseModel(1, "Thành công !", roleActions));
  } catch (error) {
    let response = new ResponseModel(404, error.message, error);
    res.status(404).json(response);
  }
